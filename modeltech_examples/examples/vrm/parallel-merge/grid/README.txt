...
merge process.
It is a requirement that it is possible to submit jobs to the grid that is to
be used. This example uses Sungrid (SGE) as the load sharing software. VRM and 
therefore the parallel merge can integrate with any load sharing system but 
supports SGE and LSF out of the box.
The process will look into current directory and recursively search
...
There are two other parameters that control how the merge is split, the
first in JOBS which control how many first level jobs the merge is broken
down into from the total.  For example if JOBS is set to 4 then the total
number of UCDBs to merge is divided into 4 JOBS. The second parameter is the
GRIDSLOTS parameter which controls how many job slots are available on the
grid. This will cause VRM to only ever launch the GRIDSLOT number of jobs,
and it will continue to manage this during the whole merge process. In the
example the JOBS and GRIDSLOTS are set to the same number but having the
parameters separate gives more control over the first level merges.
